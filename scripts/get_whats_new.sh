...
issues=$(curl -H "Authorization:token ${github_comment_bot_api_key}"  -X GET "https://api.github.com/repos/suhocki/starlord/pulls?state=closed&per_page=1" | jq '[.[]][0]' | jq -r '.body' | grep -oP '(?<=#)[0-9]+')
